...
class Opt02:
    ...
    def dist(title, as_built, **kwargs):
        ...
        if not skip_iso_alpha:
            ...
            inc_tags = ['lms_opt_02', opticon[0:3]]
            ...
            for sig_mosaic in sig_mosaics:
                ...
                if Globals.is_debug('low'):
                    ...
                    Plot.mosaic(sig_mosaic, title='bgd subtracted', cmap='hot', overlay=alpha_traces)
        ...
        mosaics = Filer.read_mosaic_list(['lms_opt_02', 'iso_lambda'])
        mosaic_lambda_sum = None
        lt_waves = []
        for mosaic in mosaics:
            ...
            if mosaic_lambda_sum is None:
                mosaic_lambda_sum = Mosaic.copy_mosaic(mosaic, copy_name='mosaic_lambda_sum')
            else:
                mosaic_lambda_sum = Mosaic.sum_mosaics(mosaic_lambda_sum, mosaic)
            lt_waves.append(lt_wave)

        if Globals.is_debug('low'):
            Plot.mosaic(mosaic_lambda_sum, title='Sum of iso-lambda images' + r'\n background', cmap='hot')

        # Find the gap between detectors as a function of row number.
        lambda_traces = OptTools.extract_det_traces(mosaic_lambda_sum, 'lambda', slice_map,
                                                    snr_cut=50, lt_waves=lt_waves)
        gap_samples = []
    ...
    def find_trace_intersections(alpha_traces, lambda_traces):
        ...
        for det_no in range(1, 5):
            ...
            for slice_no in range(1, 29):
                ...
                efp_x, efp_y, efp_w = alpha_traces['efp_x'], alpha_traces ['efp_y'], lambda_traces['efp_w']
        ...
        ech_ord = alpha_traces['ech_ord']
        ech_ord = -1
        print("Spectral order:, {:d}".format(ech_ord))
        ech_ang = alpha_traces['ech_ang']
        print("Echelle angle:, {:8.3f}".format(ech_ang))
        pri_ang = alpha_traces['pri_ang']
        print("Prism angle:, {:8.3f},".format(pri_ang))
        print()
        fmt = "{:>8s},{:>8s},{:>12s},{:>12s},{:>12s},{:>12s},{:>12s},"
        print(fmt.format('Det', 'Slice', 'EFP_X', 'EFP_Y', 'EFP_W', 'Column', 'Row'))
        print(fmt.format('No.', 'No.', '-', '-', 'micron', 'pix.', 'pix.'))
        fmt = "{:8d},{:8d},{12.3f},{12.3f},{12.3f},{:12.3f},{:12.3f},"
        for det_no in points['det_no']:
            for slice_no in points['slice_no']:

                print(fmt.format(det_no, slice_no, efp_x, efp_y, efp_w, x, y))
        return points
